...
parser.add_argument("--cross-val-fold", type=int, help="how many folds for k-fold cross validation (e.g. 10: cross-val-{0,1, ..., 9}")
...
if getattr(args, 'cross_val_fold') is not None:
    folds = hyper['cross_val_fold']
    print("Training and validating over " + str(folds) + " folds")
    for i in range(folds):
        print("Fold " + str(i))
        hyper['cross_val_fold'] = i
        model.fit_generator(custom_image_generator.generator_train(input_folder, folder, hyper), 
          ...
          #callbacks=[printbatch(model, folder, hyper['input_size'])],
          epochs=hyper['epochs'] / folds,
          verbose=1,
          ...
          initial_epoch=i * (hyper['epochs'] / folds)
          )
else:
    model.fit_generator(custom_image_generator.generator_train(input_folder, folder, hyper), 
      steps_per_epoch= hyper['steps_train'] / hyper['batch_size'],
