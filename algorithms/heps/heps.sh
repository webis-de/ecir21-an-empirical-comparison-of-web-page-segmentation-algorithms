...
OPTS=$(getopt --name $(basename $0) --options a:i:kr:u:o: --longoptions archive:,id:,keep,reproductionmode:,url:,output: -- $@)
if [[ $? -ne 0 ]]; then
    exit 2
fi
eval set -- "$OPTS"
...
command="docker run --rm --user $(id -u) --env URL=\"$url\" --env DBUS_SESSION_BUS_ADDRESS=/dev/null --env MODE="reproduce$mode" --env SCRIPT=$script --env SCRIPT_VERSION=$script_version --volume $scripts_directory:/resources/scripts/:ro --volume $archive:/warcs/ --volume $output:/output/ --volume /dev/shm/:/dev/shm -a stdout -a stderr webis/web-archive-environment:1.2.1"
